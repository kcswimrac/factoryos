...
router.get('/:nodeId/vendor', async (req, res) => {
  ...
  try {
    const [vendorRows, cutsheetsRows] = await Promise.all([
      pool.query(
    ...
    await pool.query(
      `INSERT INTO node_vendor_info
       ...
       ON DUPLICATE KEY UPDATE
         ...
         sourcing_status    = VALUES(sourcing_status),
         updated_at         = NOW()`,
...
router.get('/:nodeId/cutsheets', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, cutsheets: rows });
  } catch (e) {
...
router.post('/:nodeId/cutsheets', requireRole('editor'), async (req, res) => {
  ...
  try {
    const [posRows] = await pool.query(
      'SELECT COALESCE(MAX(position), -1) + 1 AS next_pos FROM node_cutsheets WHERE node_id = ?',
      [nodeId]
    );
    ...
    const [insertedRows] = await pool.query(
      `SELECT id, node_id, label, file_name, mime_type, file_size, position, created_at
       FROM node_cutsheets WHERE id = ?`,
      [result.insertId]
    );
    res.json({ success: true, cutsheet: insertedRows[0] });
  } catch (e) {
...
router.patch('/:nodeId/cutsheets/:cutsheetId', requireRole('editor'), async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE node_cutsheets SET label = ? WHERE id = ? AND node_id = ?',
...
router.delete('/:nodeId/cutsheets/:cutsheetId', requireRole('editor'), async (req, res) => {
  ...
  try {
    await pool.query(
      'DELETE FROM node_cutsheets WHERE id = ? AND node_id = ?',
      [cutsheetId, nodeId]
    );
    res.json({ success: true });
  } catch (e) {
